...
\item ...
\end{itemize}
A one-place predicate such as \emph{player} or \emph{man} is interpreted as a function from individuals to truth-values. Intensionality is everywhere implicit, due to the fact that the denotations of English expressions can refer to stochastic functions such as \lstlinine{gender}.
\begin{itemize}
\item \sv{player} = \lstinline{($\lambda$ (x) (element? x player))}  
\item \sv{team} = \lstinline{($\lambda$ (x) (element? x teams))}
\item \sv{match} = \lstinline{($\lambda$ (x) (element? x matches))}
\item \sv{man} = \lstinline{($\lambda$ (x) (equal? (gender x) `male))}
\item \sv{woman} = \lstinline{($\lambda$ (x) (equal? (gender x) `female))}
\end{itemize}
\todo[inline]{We need to fix the lstinline environment so that \$$\backslash$lambda\$ and \$$\backslash$theta\$ display as $\lambda$ and $\theta$.}
Predicates denote intensions of properties: they pick out functions from individuals to truth-values in any world, but the specific function that they pick out in a world depends on the random choices (e.g., evaluations of \lstinline{flip}) that are made in the process of constructing the world. In these examples, \emph{player} is true of the same individuals in every world, since \lstinline{player} is defined deterministically in the intuitive theory in figure \ref{theory}. \emph{man}, on the other hand, denotes a predicate which will be true of a given individual (say, \lstinline{`Bob}) in 50\% of partial worlds constructed (assuming no relevant observations have been made). This is because the definition of \lstinline{man} requires that the \lstinline{gender} function return \lstinline{`male} when applied to the interpretation of its syntactic sister, and \lstinline{gender} is defined in figure \ref{theory} as a stochastic function which returns \lstinline{`male} and \lstinline{`female} with equal probability. 

Similarly, transitive verbs such as \emph{won} and relational nouns such as \emph{captain} and \emph{team} denote the (stochastic) intensions of two-place predicates. (We simplify by ignoring tense.)
\begin{itemize}
\item \sv{won} = \lstinline{($\lambda$ (match) ($\lambda$ (x) (equal? x (winner match))))}
\item \sv{captain} = \lstinline{($\lambda$ (team) ($\lambda$ (x) (equal? x (captain team))))}
\end{itemize}
...
\begin{itemize}
\item \sv{strong} = \lstinline{($\lambda$ (x) ($\lambda$ ($\theta$) (> (strength x) d)))}
\item \lstinline{(define (weakness x) (- 0 (strength x))))}
\item \sv{weak} = \lstinline{($\lambda$ (x) ($\lambda$ ($\theta$) (> (weakness x) d)))}
\end{itemize}
...
Finally, we define a small amount of functional vocabulary---some quantificational determiners and the degree operator \emph{-est}---along with domains for their application.
\begin{itemize}
\item \lstinline{(define domain-of-individuals (union player team match))}
\item \sv{all} = \lstinline{($\lambda$ (P) ($\lambda$ (Q) (all (map ($\lambda$ (x) (not (and (P x) (not Q x)))) domain-of-individuals))))}
\item \sv{some} = \lstinline{($\lambda$ (P) ($\lambda$ (Q) (some (map ($\lambda$ (x) (and (P x) (Q x))) domain-of-individuals))))}
\item \sv{\text{-}est} = \lstinline{($\lambda$ (N) ($\lambda$ (A) (lambda (x) (= (argmax A x) (argmax N A)))))}
\item \sv{be\ on} =  \lstinline{($\lambda$ (team) ($\lambda$ (x) (element? x (players-on-team team))))} 
\item \sv{the} = \lstinline{($\lambda$ (P) ($\lambda$ (x) (P x)))} 
\end{itemize}
...
\item [$=$] \lstinline{((($\lambda$ (match) ($\lambda$ (x) (equal? x (winner match)))) `match1) `team1)}
\item [$\equiv$] \lstinline{(equal? `team1 (winner `match1))}
\end{itemize}
\end{itemize}
The literal listener's posterior belief state after these utterances will thus be given by:
