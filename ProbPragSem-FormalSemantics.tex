...
English lexical items are interpreted as Church expressions, and sentence meanings are built up recursively from the denotations of lexical items along lines provided by the syntactic structure. The function mapping English expressions to their interpretation in Church is written \denot{\cdot}.\footnote{In a full Church implementation, each lexical specification ``\denot{English\text{-}expression} = \lstinline{church-expression}'' would be incorporated into a single \lstinline{lexicon} function which is defined casewise. We use the notation here because it is familiar to linguists, and it allows us to give a partial specification of the lexicon without excluding the possibility that this function is able to interpret additional items.}   

The stochastic $\lambda$-calculus implemented by Church thus functions as our intermediate translation language, just as the simply-typed $\lambda$-calculus functions as an intermediate translation language in Montague's \citeyear{montague73} fragment of English. A key difference, however, is that the intermediate level is not merely a convenience as in Montague's approach. Conceptual representations and world knowledge are also represented as Church expressions, and use of a common language to represent linguistic and non-linguistic information makes it possible to model intricate interactions between these two types of knowledge. 

For example, the interpretation of a name is a Church expression which is uniquely associated with some object or individual (the name-bearer). Interpreted in this way, names are directly referential since their reference does not depend on stochastic choices made in the interpretation process.
\begin{itemize}
