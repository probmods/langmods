...
\subsection{Interpreting English in the Probabilistic Language of Thought}
...
\item ...
\end{itemize}
A one-place predicate such as \emph{player} or \emph{man} is interpreted as the characteristic function of a list ($\approx$ set) of individuals.\footnote{\lstinline{(element? x lst)} is defined as \lstinline{(not (null? (memq x lst)))}.}
\begin{itemize}
...
\item \sv{\text{-}est} = \lstinline{($\lambda$ (N) ($\lambda$ (A) (lambda (x) (= (A x) (argmax N A)))))}
\end{itemize}
The interpretations of \emph{all} and \emph{some} are standard. \emph{-est} is defined so that, for example, \emph{strongest player} is true of Bill if and only if Bill's degree of strength is equal to the maximal degree of strength of any player.
