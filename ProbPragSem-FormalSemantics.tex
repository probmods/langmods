...
\item match: a function $D_e \rightarrow \{0,1\}$, true of matches only, with an optional integer argument (since we want both _a match_ and _in match n_) 
...
\item the: a (stochastic) function from properties to individuals who satisfy the property. We could make it a hard max, but I think softmax would be more appropriate, since \emph{the dog} is supposed to be bad not only when there are two equally salient dogs, but also gradedly bad when there are two dogs of which one is just more salient than the other.
