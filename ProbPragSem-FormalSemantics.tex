...
English lexical items are interpreted as Church expressions, and sentence meanings are built up recursively from the denotations of lexical items along lines provided by the syntactic structure. The function mapping English expressions to their interpretation in Church is written \denot{\cdot}. 
...
\item ...
\end{itemize}
A non-gradable, one-place predicate such as \emph{player} is interpreted as a list of individuals or as the characteristic function of a list of individuals. 
\begin{itemize}
\item \sv{player} = \lstinline{player} = \lstinline{`(Bob Jim Alice Sue)}
\item \sv{team} = \lstinline{teams} = \lstinline{`(team1 team2 team3)}
\item \sv{matches} = \lstinline{matches} = \lstinline{`(match1 match2)}
\item \sv{man} = \lstinline{($\lambda$ (x) (equal? (gender x) `male))}
\item \sv{woman} = \lstinline{($\lambda$ (x) (equal? (gender x) `female))}
\end{itemize}
\todo[inline]{We need to fix the lslinline environment so that \$$\backslash$lambda\$ displays as $\lambda$.}
Predicates denote stochastic intensions of properties: they denote sets of individuals in any world, but may do so non-deterministically, depending on the random choices that are made in the process of constructing a partial world. In these examples, \emph{player} denotes the same list of individuals in every world. \emph{man}, on the other hand, denotes a predicate which---in the prior---will be true of a given individual (say, \lstinline{`Bob}) in 50\% of partial worlds constructed; this is because it requires that the \lstinline{gender} function return \lstinline{`male} when applied to that individual, and \lstinline{gender} is defined so that this will occur half of the time in the initial distribution. 
...
Gradable adjectives such as \emph{strong} and \emph{weak} denote relations between individuals and degrees \citep[][etc.]{cresswell76,vonstechow84}, differing only in the directionality of the comparison operator. 
\begin{itemize}
\item \sv{strong} = \lstinline{($\lambda$ (x) ($\lambda$ (d) (> (strength x) d)))}
\item \sv{weak} = \lstinline{($\lambda$ (x) ($\lambda$ (d) (< (strength x) d)))}
\end{itemize}

Finally, we define a small amount of functional vocabulary 
