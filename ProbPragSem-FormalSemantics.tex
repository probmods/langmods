...
(define (winner match) 
  (let ([teamA (first (teams-in-match match))]
    ...
    (if (< (team-pulling teamA) (team-pulling teamB))
        ...
        teamB)))
\end{lstlisting}
Notice that the team pulling is simply the sum of how hard each member is pulling; each player pulls with their intrinsic strength, plus a random amount that indicates their effort on this match. \todo{i have left laziness implicit. do we need it anywhere?}

Altogether the conceptual lexicon of functions for the tug-of-war domain is given in Figure \ref{theory}.
...
\end{figure} 
 ...
 \todo[inline]{
-Basic effects: Explaining away + screening off. 
  -Observe sam beats jim, infer sam is stong. learn that jim is weak, explain away match outcome, so sam is ordinary strength again.
  -Observe sam beats jim, predict sam will beat jim again. Knowing jim and sam's strengths, finding out sam beat jim doesn't change prediction. 
  -Mention but don't illustrate other effects ---  Size principle / occam, transfer learning.
  
-Modularity vs joint inference: knowledge is modular, inference is not.
}
