...
The left panel of figure \ref{quantifiers} plots the strength of each reading as the number of matches varies. 
The right panel shows the mean inferred team size as the number of matches varies. 
\begin{figure}[tbh]
\begin{center}
\includegraphics[scale=.6]{quantifiers.pdf}
\end{center}
\label{quantifiers}
\caption{The probability of assigning each of the two readings of \emph{Most players played in some match} depends in intricate ways on the interpreter's beliefs and observations about the number of matches and the number of players on each team.}
\end{figure} 
Note that the inferred team size is insensitive to variation in the number of matches when the object wide scope reading is assigned: this is because the existence of a single match with most players in it entails that team size is 4, 5, or 6 no matter how many matches were played. 
