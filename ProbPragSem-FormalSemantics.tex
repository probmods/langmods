...
Language is used to communicate ideas. Ideas are mental tools for coping with a complex and uncertain world. This implies that human conceptual structures should be key to language meaning, and that probability, the mathematics of uncertainty, should be indispensable for describing both language and thought. Indeed, probabilistic models are enormously useful in modeling human cognition \citep{tenenbaumetal11}
%Tenenbaum, etc}, 
...
\todo{Correct citation for de finetti?}
...
\todo{mention fuzzy logic (too much independence), and shalom and van eijk's recent work (no compositionality to background distribution).}
...
In many contexts, it would also be taken to suggest that Jane did not play in all of the matches, either. 
...
Quantity implicatures are predicted in our model because the pragmatic listener uses counterfactual reasoning to help reconstruct the speaker's belief state from his actual choice of utterance.
Suppose that the QUD is ``How many matches did Jane play in?''. Then alternative utterance ``Jane played in all the matches'' would have be appropriate if it were in fact true. Then the speaker's preference for informative utterance would have caused him to (probabilistically) prefer this utterance to the less informative ``Jane played in some matches'' if it were true. 
This preference is generated by the fact that the probability that a knowledgeable speaker will choose utterance $u$ (given question under discussion $Q$) is determined in large part by the probability that the listener will choose the correct answer to $Q$ if the speaker chooses $u$.
Since the speaker in fact chose the less informative utterance in this case, the listener can use Bayesian reasoning to infer that some precondition for the stronger utterance's use---e.g., its truth---is probably not fulfilled. 
...
In this way, our model yields a broadly Gricean explanation of quantity implicature, but without stipulating ad-hoc rules of conversation. Instead, the content of Grice's Maxim of Quantity falls out of the recursive pragmatic reasoning process whenever it is reasonable to assume that the speakers is making an effort to be informative. (For related formal reconstructions of Gricean reasoning, see \citet{franke09,Vogel-etal:2013}.)
...
\end{figure}
The interpretation of the free degree variable has indeed been influenced by context: it changes from its prior to a posterior that makes the utterance likely to be true. 
...
In addition, the listener has learned nothing at all about the variable of interest from the utterance: the posterior distribution on Bob's height is the same as the prior.
...
This model of gradable adjective interpretation (which was introduced in \cite{lassitergoodman13}) has a number of appealing properties: for instance, it explains how background knowledge affects qualitative and quantitative aspects of adjective interpretation (see \cite{lassitergoodman13}), and it explains why vagueness is a pervasive feature of adjective interpretation (see below).
...
%
% vagueness – of interest to linguists, philosophers, psychologists, cs, etc.
% say what sorites is, why it's interesting
% 
% bill is strong – from above
% if x is just a little weaker than y and x is strong then y is strong
% so, everyone is strong
% show conclusion can have low prob 
