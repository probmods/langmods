...
A more productive strategy is the one we have taken: world knowledge specifies a joint distribution on Bob and Jacks strengths (by first sampling the prototypical strength of men, then sampling the strength of each), and the sentence imposes a \emph{constraint} on this distribution (that each man's strength exceeds a threshold). 
...
Coping with (e.g.) gaps and binding will thus require additional combinatorial operations: see  \citep{jacobson99,steedman00,barker05} for discussion.
...
Note that each lexical item can is associated with an infinite number of (static) types, the probability of associating an item with a type declines as an exponential function of the number of type-raising operations required. 
The mechanism for generating type ambiguities thus automatically enforces the preference for interpretation in lower types, a feature which is often stipulated in discussion of type-shifting \citep{parteerooth83,partee87}.
