...
At one extreme, suppose that the listener has observed that each team has two individuals in it and  (as the model in figure \ref{theory} already implies) that six matches were played:
\begin{lstlisting}
(query
  ...ToW theory...
  (eval QUD)
...
\end{lstlisting}
Given that there are twelve players, the reading with \emph{some match} taking wide scope is clearly false in this scenario: only four players could be in any given match. However, the reading with \emph{every player} taking wide scope could be true in this scenario if no one played twice. 
%
% cont from here
% 
