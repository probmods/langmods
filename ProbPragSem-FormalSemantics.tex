...
However, the situation changes dramatically if Jane in fact played in all the matches: now the speaker prefers the more informative utterance ``Jane played in all the matches'' by a factor of about 3. This is because the choice of this utterance over the less informative alternative increases the probability that the listener will assign to the true answer to the QUD (3) by this factor. 
...
\begin{figure}[tbh]
%\begin{center}
\includegraphics[scale=.3]{SI.pdf}
%\end{center}
%\label{GAs-L0}
\caption{Three stages of the interpretation of ``Jane played in some matches'', assuming that the only relevant alternative is ``Jane played in all the matches''. Note that the while the literal listener (left pane) assigns a moderate probability (.3) to the ``all'' situation given this utterance, the pragmatic listener (right pane) assigns this situation a very low probability, approximately .09. The difference is due to the fact that the pragmatic listener reasons about the utterance choices of the speaker (center pane).}
\end{figure}
