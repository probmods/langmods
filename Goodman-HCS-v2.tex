...
In \citep{FG12} the RSA model was applied to explain the results of simple reference games in which a speaker attempted to communicate one of a set of objects to a listener by using a simple property to describe it (e.g.~\emph{blue} or \emph{square}). Here the intuitive theory can be seen as simply a prior distribution, \lstinline{(define ref (ref-prior objects))} over which object is the referent in the current trial, the \lstinline{QUD} is simply \lstinline{ref}, and the properties have their standard extensions. By measuring the \lstinline{ref-prior} empirically \citet{FG12} were able to predict the speaker and listener judgements with high quantitative accuracy (correlation around 0.99).

In \citet{Goodman13} the RSA framework was extended to take into account the speaker's knowledge state. In this case the speaker should choose an utterance based on it's \emph{expected} informativity given the speaker's belief distribution. (Or, equivalently, the utility is based on the KL divergence of the listener's posterior beliefs from the speaker's.) This extended model makes the interesting prediction that listeners should not draw strong quantity implicatures from utterances by ignorant speakers. The experiments in \citet{Goodman13} show that this is the case, and that the quantitative predictions of the model are borne out.

As a final example of extensions to the RSA framework, the \lstinline{QUD} itself can be an object of inference. If the pragmatic listener is unsure what topic the speaker is addressing, as must often be the case, then she should jointly infer the \lstinline{QUD} and it's \lstinline{val} under the assumption that the speaker chose an utterance to be informative about the topic (whatever it is). This simple extension can lead to striking predictions. In \citet{kao-metaphor,kao-hyperbole} such \lstinline{QUD} inference was shown to give rise to non-literal interpretations: hyperbolic and metaphoric usage.
...
It is most similar to game theoretic approaches to pragmatics. In particular to approaches that treat pragmatic inference as iterated reasoning, such as the Iterated Best Response (IBR) model \citep{franke,jaeger}. The IBR model represents speakers and listeners recursively reasoning about each other, as in the RSA model. The two main differences are that IBR specifies unbounded recursion between speaker and listener, while RSA as presented here specifies on one level, and the IBR specifies that optimal actions are chosen, rather than soft-max decisions. Neither of these differences is critical to either framework. We view it as an empirical question what degree of optimality and what level of recursive reasoning people actually display in language understanding.
