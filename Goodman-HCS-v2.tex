...
(define domain-of-individuals (union players teams matches))
...
For instance, \emph{player} is true of the same individuals in every world, because \lstinline{players} is defined deterministically in the intuitive theory (Figure \ref{tow}) and \lstinline{element?} is the deterministic membership function. 
\todo{this description of man is super wordy.. simplify}
\emph{man}, on the other hand, denotes a predicate which will be true of a given individual (say, \lstinline{`Bob}) in 50\% of worlds (assuming no relevant observations have been made). 
...
Quantifiers and the superlative morpheme: \\
 \sv{every} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (all (map ($\lambda$ (x) (not (and (P x) (not Q x)))) domain-of-individuals))))}\\
 \sv{some} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (some (map ($\lambda$ (x) (and (P x) (Q x))) domain-of-individuals))))}\\
 \sv{most} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (> (proportion-true (map ($\lambda$ (x) (and (P x) (Q x))) domain-of-individuals)) .5)))}\\
...
To be concrete, consider the case of gradable adjectives like \emph{strong}. In section \ref{lexicon} we have defined \sv{\mathit{strong}} = \lstinline[mathescape]{($\lambda$ (x) ($\lambda$ ($\theta$) (>= (strength x) $\theta$)))}, without specifying how \lstinline[mathescape]{$\theta$} is bound in sentences such as \emph{Bob is strong}.
