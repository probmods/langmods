...
\end{itemize}
Note that the denotation of \emph{played in} takes a list of individuals as its second argument, but that the order of elements in the list does not matter.
\todo{where is this set-based plural business used? this def of played in is kind of messy. do we need a shifter that can make a list from an individual?}
...
We take gradeable adjectives, such as \emph{strong}, to denote simply functions from individuals to degrees \citet[Cf.][]{neeleman2004degree}. 
\begin{itemize}
...
\subitem \lstinline{meaning}: \lstinline[mathescape]{((R ((R 'team 1) (($\lambda$ (x) ($\lambda$ (y) (list x y))) 'team2))) (($\lambda$ (match) ($\lambda$ (team-list) (all (map ($\lambda$  (x) (element? x (teams-in-match match)))) team-list))) 'match1))}
...
In the absence of pragmatic reasoning (described below), this is equivalent to conditioning on the conjunction of the meanings of each utterance.  
