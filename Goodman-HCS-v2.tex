...
%\todo[inline]{Do we want to give a definitional interpreter (eval) for ESLC in an appendix???}



\subsection{Commonsense knowledge}

\todo[inline]{Need to make sure the Church code accurately reflects the simulation setup.}
...
(define individuals (union players teams matches))
...
The conceptual lexicon of functions for the tug-of-war domain is given altogether in Figure \ref{tow}.
...
The above definition for \lstinline{meaning} always results in composition by forward application; this is too limited to generate potential meanings for many sentences. For instance ``Bob runs.'' requires a \emph{backward} application to apply the meaning of ``runs'' to that of ``Bob''. We extend the possible composition methods by allowing the insertion of type-shifting operators. See  \citep{jacobson99,steedman01,barker05} for discussion of such approaches.
\begin{lstlisting}
(define (meaning utterance)
  (if (lexical-item? utterance)
      (lexicon utterance)
...
\end{lstlisting}
Each intermediate meaning will be shifted zero or more times by a randomly chosen type-shifter; because the number of shifts is determined by a stochastic recursion, fewer shifts are \emph{a priori} more likely (with no shift being most likely of all). 
Each lexical item thus has the potential to be interpreted in any of an infinite number of (static) types; however, the probability of associating an item with an interpretation in some type declines exponentially with the the number of type-raising operations required to construct this interpretation. 
...
Type shifters \textbf{AR1} and \textbf{AR2} allow flexible quantifier scope as described in \citet{hendriks93,barker05} (the specific choice here follows \citet[][pp.453ff.]{barker05}). This treatment of quantifier scope is convenient, but others could be implemented by complicating the syntactic or semantic mechanisms in various ways: see e.g.~\citet{may77,steedman2012taking}.
...
%In this section we sketch out a formal fragment of English appropriate for talking about the tug-of-war domain described earlier. The complete lexicon is given in appendix \ref{tow-lexicon}, for reference.

For simplicity we write the denotations of utterances and words, \lstinline[mathescape]{(meaning $\cdot$)} and \lstinline[mathescape]{(lexicon $\cdot$)}, both as \denot{\cdot}. A summary of the lexicon we will use for the tug-of-war domains is shown in Figure \ref{tow-lexicon}.
...
For example, 
\sv{man} = \lstinline[mathescape]{($\lambda$ (x) (equal? (gender x) `male))} or \sv{player} = \lstinline[mathescape]{($\lambda$ (x) (element? x players))}.
Similarly, transitive verbs such as \emph{won} and relational nouns such as \emph{captain} denote two-place predicates. For instance, \sv{won} = \lstinline[mathescape]{($\lambda$ (match) ($\lambda$ (x) (equal? x (winner match))))} (we simplify throughout by ignoring tense).
%
...
On the other hand, \emph{man} denotes a predicate which will be \emph{a priori} true of a given individual (say, \lstinline{`Bob}) in 50\% of worlds---because \lstinline{gender} returns \lstinline{`male} in half of the possible worlds.
%This is because the denotation of \emph{man} is a function which returns true if and only if its argument is an individual for which the \lstinline{gender} function returns \lstinline{`male}. 
%The \lstinline{gender} function, in turn, is defined %in figure \ref{theory} 
...
We can adopt standard meanings for functional vocabulary, such as quantifiers: 
\sv{every} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (subset? P Q)))}.
(The function \lstinline{subset?} can be defined in the standard way given a domain of discourse: \lstinline{(define (subset? P Q) (null? (filter (not Q) (filter P domain))))}. The \lstinline{domain} is defined in the background knowledge, Figure \ref{tow}.)
%\sv{every} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (all (map ($\lambda$ (x) (not (and (P x) (not Q x)))) individuals))))}.
These definitions are meant to be illustrative, rather than definitive: many alternative definitions of these items could have been used instead to illustrate the theory of language understanding described here.
...
We take gradeable adjectives, such as \emph{strong} and \emph{weak}, to denote simply functions from individuals to degrees. For instance, \sv{strong} = \lstinline[mathescape]{($\lambda$ (x) (strength x))}. 
...
For instance, the superlative morpheme \emph{-est} is defined so that, for example, \emph{strongest player} will denote a property that is true of an individual if and only if that individuals strength is equal to the maximum strength of all players: 
\sv{\text{-}est} = \lstinline[mathescape]{($\lambda$ (A) ($\lambda$ (N) ($\lambda$ (x) (= (A x) (max A N)))))}.
For the positive form, \emph{strong}, we will employ in section \ref{indices} a type shifting operator which introduces a degree variable.
...
\todo{Shalom: Explain how this integration of different sources of uncertainty through conditioning, to update a probability value for stength, works.}
...
\item Lexical: the \lstinline{lexicon} function may be stochastic, returning different options for a single item, or words may have intrinsically stochastic meanings (and the former may be converted to the latter). 
\end{itemize}
...
\end{itemize}
With \lstinline{random-combinator} defined as above, we predict the two readings of of \emph{Most players played in some match} as follows:
\begin{itemize}
...
The left panel of the figure \ref{scope-ambiguity}
...
It should be clear that compositionally has played a key role in our model of language interpretation thus far. It has in fact played several key roles: expressions in SLC are built from simpler expressions, sequences of utterances are interpreted by sequential conditioning, the \lstinline{meaning} function composes SLC expressions to form sentence meanings. There are thus several, interlocking ``directions'' of compositionality at work, and they result in interactions that could appear non-compositional if only one direction was considered. Let us focus on two: compositionality of world knowledge and compositionality of linguistic meaning.
...
\todo{Put in formal QUD.. \lstinline{(num-true (element? `jane (map players-on-team (teams-in-match matches)))}}. 
...
\todo[inline]{Noah: fill in pragmatics extensions and related work.}
...
\todo[inline]{semantic indices as the result of combinators? eg. maybe we should embrace POS insertion as necessary to convert from e->d to e->t...}
...
To be concrete, consider the case of gradable adjectives like \emph{strong}. In section \ref{lexicon} we have defined \sv{\mathit{strong}} = \lstinline[mathescape]{($\lambda$ (x) (strength x))}; to form a property out of a positive form in a sentence like \emph{Bob is strong.} we must bind the degree in some way.
A simple way to do this is to add a ``type-shifter'' that introduces a free threshold variable $\theta$:
\lstinline[mathescape]{(define type-shifters `(R S AR1 AR2  T...))}, with:
\begin{itemize}
\item \textbf{T}:  \lstinline[mathescape]{($\lambda$ (A) ($\lambda$ (x) (> (A x) theta)))}
\end{itemize}
...
Taking this simplest well-typed meaning, the utterance ``Bob is strong'' (with \lstinline{QUD} ``how strong is Bob?'') would be interpreted via:
\begin{lstlisting}
(query
  ...theory...
  (define $\theta$ (uniform))
...
Quantifiers and the superlative morpheme:
\begin{itemize}
\item \lstinline{(define individuals (union player team match))}
