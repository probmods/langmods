...
For simplicity, we assume that the conjunction of NPs forms a list, \sv{and} = \lstinline[mathescape]{list}; for instance, \emph{Team 1 and Team 2} denotes  \lstinline{(list `team1 `team2)}. Compare the set-based account of plurals described in Scha \& Winter, this volume.
...
\end{itemize}
For simplicity we have written the quantifiers in terms of set size; the \lstinline{size} function can be defined in terms of the \lstinline{domain} of individuals as \lstinline[mathescape]{($\lambda$ (S) (length (filter S domain)))}.
