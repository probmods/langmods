...
The above definition for \lstinline{meaning} always results in composition by forward application; this is too limited to generate potential meanings for many sentences. For instance ``Bob runs.'' requires a \emph{backward} application to apply the meaning of ``runs'' to that of ``Bob''. We extend the possible composition methods by allowing the meanings to be type-shifted as they are computed. See  \citep{jacobson99,steedman01,barker05} for discussion of such approaches.
\begin{lstlisting}
(define (meaning utterance)
  (if (lexical-item? utterance)
      (lexicon utterance)
...
\end{lstlisting}
Each intermediate meaning will be shifted zero or more times by a randomly chosen type-shifter; because the number of shifts is determined by a stochastic recursion, fewer shifts are \emph{a priori} more likely (with none being most likely). 
...
\todo{Shalom: Explian why 'player' is true of the same objects in all worlds, while other semantic constants vary in denotation across worlds.}
In these examples, \emph{player} is true of the same individuals in every world, since \lstinline{player} is defined deterministically in the intuitive theory (Figure \ref{tow}). 
