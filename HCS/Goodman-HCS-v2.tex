\documentclass[pdfextras]{handbook}  


%%possibly people who can provide initial feedback:
%chris potts, chris barker, tim o'donnell, ted gibson, 
%adrian brasoveanu, 
%roger levy,
%pranav anand
%raj singh, roni katzir
%greg scontras,
%ken shan
%
%hank zeevat, 
%david beaver, hans kamp,
%robin cooper, van eijk
...
\begin{document}
% The following turns on line numbering, which is useful for
% reviewers, proof readers and copy editors. Comment out this line if
% the line numbering causes problems.
%\linenumbers

\title*{Probabilistic Semantics and Pragmatics: Uncertainty in Language and Thought}
\titlerunning{Probabilistic Semantics and Pragmatics}


\author{Noah D.~Goodman\inst{1}
        and Daniel Lassiter\inst{1}}

\institute{%
   Stanford University\\
    \texttt{\{ngoodman,lassiter\}@stanford.edu}
 }
...
\todo[inline]{Extract code, make probmods page, make sure it runs! (Erin and Julius to help.)}
...
%The tools of structured probabilistic modeling, and particularly the stochastic lambda calculus we will use in this chapter, allow us to systematically construct such generative models. 
...
\todo[inline]{Shalom: Section 1.1-1.2. The transition from the general introduction in pp.1-4 to the detailed technical discussion of Church and its applications is too drastic. It needs to be mediated by informal clarification and illustration of the Church functions and
other components of Church. These subsections need to be reformulated to be accessible to a readership of formal semanticists who know classical typed lambda calculus and model theory, but who are not generally familiar with sampling, probability distrubtions, and probabilistic programming.}
...
(query
  ...
  (and (equal? (gender 'Bob) 'male) (= (strength 'Bob) -1.1)
       ...
       (= (strength 'Pat) 0.7)))
\end{lstlisting}
\todo[inline]{Erin: This doesn't work in webchurch because the tricks for having gaussians or other ERPs in the condition of queries don't work when the ERPs are hidden inside \lstinline{mem} or other stuff. can fix by using soft-equals or CFA.}
...
\end{lstlisting}
\todo[inline]{Erin: \lstinline{draw-n} not defined yet. can put in some seperate ``helper functions'' file or section or something when we put up the code}
% here's my attempt at defining draw-n:
% (define draw-n
%   (lambda
%     (n lst)
...
(define (winner match) 
  (let ([teamA (first (teams-in-match match))]
    ...
    (if (> (team-pulling teamA match) (team-pulling teamB match))
        ...
        teamB)))
\end{lstlisting}
\todo[inline]{Erin: The way this is defined, one person could be on multiple teams. E.g. there's nothing stopping Bob from being on team1, team2, and team3. This may be unavoidable...footnote?}
...
%\todo[inline]{Shalom: The first paragraph of subsection 1.2 is not clear. You need to explain how probability distributions over complete worlds can be derived from distributions over bounded sets of situations in a sampling space. This is a crucial point. You tend to use worlds and situations as interchageable, but they are not. Situation theory was introduced as an alternative to possible worlds semantics,
%and there are significant difference in the model theories associated with each of these approaches. Both classical and Bayesian probability theory tends to treat situations as worlds, but in fact bounded sets of situations (situation types defined over a bounded set of alternatives) are formally and computationally distinct from complete worlds.}

\todo[inline]{dan, is this section coherent? could also just remove it...}
...
%For the moment the meanings of the two sub-strings are combined by applying the left to the right---in a moment we will extend the possible composition methods. 
...
\todo{Acknowledge Henk Zeevat for pointing out that this system doesn't prefer the correct interpretation of "bob loves mary" without an additional syntactic bias.}

%To make this more concrete, we first extend the possible compositions to account for complex binding, 
%we then return to the tug-of-war example from the previous section and begin to build a lexicon for talking about this domain in natural language. 
...
%The purpose of this chapter is not to give a systematic treatment of compositional semantics, but we do want a system of composition which is sufficient to expose the interaction between probabilistic interpretation and compositional phenomena such as quantifier scope.
%Coping with complex linguistic phenomena requires us to complicate either or both of the syntactic (\lstinline{random-split}) or semantic (\lstinline{random-combinator}) operations. We choose to complicate the semantic component; \lstinline{random-split} is merely a device for creating binary trees from strings, and does not insert silent elements (such as indices or PRO). 
%Coping with (for example) gaps and binding will thus require additional combinatorial operations in \lstinline{random-combinator}. 
...
%For simplicity, we assume that the conjunction of NPs forms a list, \sv{and} = \lstinline[mathescape]{($\lambda$ (x) ($\lambda$ (y) (list x y)))}; for instance, \emph{Team 1 and Team 2} denotes  \lstinline{(list 'team1 'team2)}. Compare the set-based account of plurals described in Scha \& Winter, this volume.
...
\begin{itemize}
\item \sv{and} = \lstinline[mathescape]{($\lambda$ (x) ($\lambda$ (y) (list x y)))}  \todo{Erin: \lstinline{and} is already defined in a different way. Can we call this one something else?} 
\end{itemize}
...
\item \sv{some}: \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (< 0 (size (intersect P Q)))))}  \todo{Erin: \lstinline{some} is already defined in a different way. Can we call this one something else?} 
\item \sv{most}: \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (< (size P) (* 2 (size (intersect P Q))))))}
\end{itemize}
For simplicity we have written the quantifiers in terms of set size; the \lstinline{size} function can be defined in terms of the \todo{Erin: should this be \lstinline{individuals}?} \lstinline{domain} of individuals as \lstinline[mathescape]{($\lambda$ (S) (length (filter S domain)))}.\footnote{In the examples below, we assume for simplicity that many function words, for example \emph{is} and \emph{the}, are semantically vacuous, i.e., that they denote identity functions.}
...
\end{itemize}
(Note that we assume the set operator \lstinline{max} \todo{Erin: \lstinline{max} is already defined in a different way. Can we call this one something else?} implicitly quantifies over the domain of discourse.)
...
%For example: 
%\sv{every} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (subset? P Q)))}.
%Where the function \lstinline{subset?} is be defined in the obvious way given a domain of discourse: \lstinline{(define (subset? P Q) (null? (filter (not Q) (filter P domain))))}. The \lstinline{domain} is defined in the background knowledge, Figure \ref{tow}.
%\sv{every} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (all (map ($\lambda$ (x) (not (and (P x) (not Q x)))) individuals))))}.
%Alternative definitions of these items could have been used instead to illustrate the theory of language understanding described here.

%\todo[inline]{i think GAs can be simplified if we allow a type-shifter Atheta = (lambda (f) (lambda (x) (> (f x) theta))), which can bind the degree. est can then also be simplified into a simple argmax over a set...}
...
%For instance, \sv{strong} = \lstinline[mathescape]{($\lambda$ (x) ($\lambda$ ($\theta$) (>= (strength x) $\theta$)))}, where $\theta$ is a variable over degrees (or ``thresholds'').
...
% \sv{\text{-}est} = \lstinline[mathescape]{($\lambda$ (A) ($\lambda$ (N) ($\lambda$ ($\theta$) (= (argmax A x) (argmax ($\lambda$ ($\theta$) (some (map ($\lambda$ (x) ((A x) $\theta$)) N))))))))}.

%\todo[inline]{Leave the threshold as an unbound variable, and define -est to bind this variable before doing its work. discuss with dan... }
...
%Entities:\\
%\sv{Bob} = \lstinline{'Bob} \\
%\sv{Team\ 1} = \lstinline{'team1} \\
%\sv{Match\ 1} = \lstinline{'match1} \\
%...
%\end{minipage}
%\begin{minipage}{0.6 \textwidth}
%Properties:\\
% \sv{player} = \lstinline[mathescape]{($\lambda$ (x) (element? x players))}  \\
% \sv{team} = \lstinline[mathescape]{($\lambda$ (x) (element? x teams))}\\
% \sv{match} = \lstinline[mathescape]{($\lambda$ (x) (element? x matches))}\\
% \sv{man} = \lstinline[mathescape]{($\lambda$ (x) (equal? (gender x) 'male))}\\
% \sv{woman} = \lstinline[mathescape]{($\lambda$ (x) (equal? (gender x) 'female))}
%\end{minipage}
%
%\begin{minipage}{1.0 \textwidth}
%Transitive verbs:\\
% \sv{won}: \lstinline[mathescape]{($\lambda$ (match) ($\lambda$ (x) (equal? x (winner match))))}\\
% \sv{played\ in}:  \lstinline[mathescape]{($\lambda$ (match) ($\lambda$ (team-list) (all (map ($\lambda$  (x) (element? x (teams-in-match match)))) team-list)))}\\
% \sv{be\ on}:  \lstinline[mathescape]{($\lambda$ (team) ($\lambda$ (x) (element? x (players-on-team team))))} \\
% \sv{captain}: \lstinline[mathescape]{($\lambda$ (team) ($\lambda$ (x) (equal? x (captain team))))}
%\end{minipage}
%
...
%\begin{minipage}{1.0 \textwidth}
%Gradable adjectives:\\
% \sv{strong} = \lstinline[mathescape]{($\lambda$ (x) ($\lambda$ ($\theta$) (>= (strength x) $\theta$)))}\\
% \sv{weak} = \lstinline[mathescape]{($\lambda$ (x) ($\lambda$ ($\theta$) (>= (- (strength x)) $\theta$)))}
%\end{minipage}
%
%\begin{minipage}{1.0 \textwidth}
%Quantifiers and the superlative morpheme: \\
% \sv{every} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (all (map ($\lambda$ (x) (not (and (P x) (not Q x)))) individuals))))}\\
% \sv{some} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (some (map ($\lambda$ (x) (and (P x) (Q x))) individuals))))}\\
% \sv{most} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (> (proportion-true (map ($\lambda$ (x) (and (P x) (Q x))) individuals)) .5)))}\\
% %\sv{\text{-}est} = \lstinline[mathescape]{($\lambda$ (A) ($\lambda$ (N) ($\lambda$ (x) (= (argmax A x) (argmax ($\lambda$ ($\theta$) (some (map ($\lambda$ (x) (>= (A x) $\theta$)) N))))))))}\\
% \sv{\text{-}est} = \lstinline[mathescape]{($\lambda$ (A) ($\lambda$ (N) ($\lambda$ (x) (= (A x) (max A N)))))}
...
%
%\caption{A fragment of English for discussing tug-of-war. These denotations depend on the concept definitions in Figure \ref{tow-theory}.}
%\label{tow-lexicon}
%\end{figure}
%
%\todo{simplify quantifier denotations...}
...
%Assume that the listener's initial belief state ( is as given in figure \ref{tow}, and his lexicon conforms to the definitions in this section. 
...
%posterior belief state after these utterances will thus be given by sequentially conditioning on the truth of the literal interpretations assigned to these utterances by the \lstinline{meaning} function. 
...
%The listener's posterior beliefs about Bob's strength (i.e.~\lstinline{(strength `Bob)}) will be identical to those shown in Figure \ref{explainingaway}b: 
...
%\item [$=$] \lstinline[mathescape]{(((($\lambda$ (A) ($\lambda$ (N) ($\lambda$ (x) (= (argmax A x) (argmax ($\lambda$ ($\theta$) (some (map ($\lambda$ (x) ((A x) $\theta$)) N)))))))) ($\lambda$ (x) (element? x player))) ($\lambda$ (x) ($\lambda$ ($\theta$) (> (weakness x) $\theta$)))) 'Jim)}
%\item [$\equiv$] \lstinline[mathescape]{(= (argmax ($\lambda$ ($\theta$) (> (weakness 'Jim) $\theta$))) (argmax ($\lambda$ ($\theta$) (some (map ($\lambda$ (x) ((weakness x) $\theta$)) ($\lambda$ (x) (element? x player)))))))    }
%\end{itemize}
%\end{itemize}
...
\todo[inline]{Erin: I don't think there's any simpler way to write this.}
...
\end{itemize}
\todo{Give the (reduced) church expressions for each of these meanings, too?}
...
%: for instance, the number of players on each team and the number of matches played.
...
%Suppose that the listener has observed that ten matches were played:
%\begin{lstlisting}
%(query
%  ...ToW theory...
%  (eval QUD)
...
%Note that the inferred team size is insensitive to variation in the number of matches when object-wide scope is used: this is because the existence of a single match with most players in it entails that team size is 4, 5, or 6 (with equal probability), no matter how many matches were played. 
...
%
%For instance, lexical ambiguities of the ``bank''/``bank'' variety could be treated either by having the \lstinline{lexicon} function map ``bank'' to either of two discrete meanings, or by assuming that the latent mechanism which divides utterances into words recognizes two distinct words with this phonological realization.
...
%However, if one of the two possible interpretations is more likely to be true than the other, the literal listener will automatically prefer the parse which generates the more probable interpretation.
...
\todo[inline]{Shalom: Complete Section 2.5, preferably by compressing it into a
brief paragraph that describes related work, with references, and
similarly for other such schematic subsections in the chapter.}
...
\end{align}
\todo{Shalom: Explain the convergence formulas in (4) and (5). (NDG: what does this mean??)}
...
\subsection{Quantity implicatures}

We illustrate by considering quantity implicatures: take as an example the sentence ``Jane played in some match''. 
...
\lstinline[mathescape]{(sum (map ($\lambda$ (m) (element? 'jane (players-in-match m))) matches))}.\todo{fix me})
%\lstinline{(num-true (element? 'jane (map players-on-team (teams-in-match matches)))}. 
...
% as long as utterances $u$ over $u'$ are assigned equal probability by \lstinline{language-prior}, the probability that a knowledgeable speaker will choose $u$ over $u'$ is determined by the probability that the listener will guess the correct answer for the \lstinline{QUD} if the speaker chooses $u$.
...
%\footnote{Note that this reasoning goes through only if the alternative set does not include more informative numerical expressions (``Jane played in exactly two matches''), or if these are dispreferred by the speaker for some reason (e.g., overinformativity). Our approach shares with other theories of implicature the need to place such restrictions on the set of relevant alternatives.} 
...
%(also do "captain" vs "player"? (would need to make names unavailable for this to work...)

%reference via joint inference (effect of prior). FG'12 ref exprs. salience. (lewis "the cat"?)


\subsection{Extensions and related work}

The simple Rational Speech Acts (RSA) framework sketched above has been fruitfully extended and applied to a number of phenomena in pragmatic understanding; many other extensions suggest themselves, but haven't yet been explored.
In \citep{frankgoodman12} the RSA model was applied to explain the results of simple reference games in which a speaker attempted to communicate one of a set of objects to a listener by using a simple property to describe it (e.g.~\emph{blue} or \emph{square}). Here the intuitive theory can be seen as simply a prior distribution, \lstinline{(define ref (ref-prior objects))} over which object is the referent in the current trial, the \lstinline{QUD} is simply \lstinline{ref}, and the properties have their standard extensions. By measuring the \lstinline{ref-prior} empirically \citet{frankgoodman12} were able to predict the speaker and listener judgements with high quantitative accuracy (correlation around 0.99).

In \citet{goodman2013knowledge} the RSA framework was extended to take into account the speaker's belief state. In this case the speaker should choose an utterance based on it's \emph{expected} informativity under the speaker's belief distribution. (Or, equivalently, the speaker's utility is the negative KL divergence of the listener's posterior beliefs from the speaker's.) 
%\begin{eqnarray}
%P_S(u|b) &\propto& e^{-KL(P(s|b), P_L(s|u))}\\
%&=& e^{-\sum_s (P(s|b) \log(P(s|b)/P_L(s|u)))}\\
%&=& e^{-\sum_s (P(s|b) \log(P(s|b)))}   e^{\sum_s (P(s|b) \log(P_L(s|u)))}\\
%&\propto& e^{\sum_s (P(s|b) \log(P_L(s|u)))}\\
%&=& e^{E_{P(s|b)}  \log(P_L(s|u))}
%\end{eqnarray}
This extended model makes the interesting prediction that listeners should not draw strong quantity implicatures from utterances by ignorant speakers. The experiments in \citet{goodman2013knowledge} show that this is the case, and the quantitative predictions of the model are borne out.

As a final example of extensions to the RSA framework, the \lstinline{QUD} itself can be an object of inference. If the pragmatic listener is unsure what topic the speaker is addressing, as must often be the case, then she should jointly infer the \lstinline{QUD} and it's \lstinline{val} under the assumption that the speaker chose an utterance to be informative about the topic (whatever that happens to be). This simple extension can lead to striking predictions. In \citet{kao-metaphor,kao-hyperbole} such \lstinline{QUD} inference was shown to give rise to non-literal interpretations: hyperbolic and metaphoric usage. 
While the literal listener will draw an incorrect inference about the state of the world from an utterance such as ``I waited a million hours'', the speaker only cares if this results in correct information about the \lstinline{QUD}; the pragmatic listener knows this, and hence interprets the utterance as only conveying information about the \lstinline{QUD}. If the \lstinline{QUD} is inferred to be a non-standard aspect of the world, such as whether the speaker is irritated, then the utterance will convey only information about this aspect and not the (false) literal meaning of the utterance: the speaker waited longer than expected and is irritated about it.

The RSA approach shares elements with a number of other formal approaches to pragmatics. 
%For instance, like other theories of implicature the set of relevant alternatives must be specified
It is most similar to game theoretic approaches to pragmatics. In particular to approaches that treat pragmatic inference as iterated reasoning, such as the Iterated Best Response (IBR) model \citep{franke09,benzetal05}. The IBR model represents speakers and listeners recursively reasoning about each other, as in the RSA model. The two main differences are that IBR specifies unbounded recursion between speaker and listener, while RSA as presented here specifies one level, and the IBR specifies that optimal actions are chosen, rather than soft-max decisions. Neither of these differences is critical to either framework. We view it as an empirical question what degree of optimality and what level of recursive reasoning people actually display in language understanding.


%\begin{itemize}
%\item Frank and Goodman 2012, ref exprs. Reference via joint inference (effect of prior); salience. (Cf. Lewis "the cat".)
%\item Extensions:
%\subitem Representing speaker knowledge \cite{Goodman2013}.
%\subitem Inferring QUD / non-literal language. more general goals?
%\subitem noisy chanel?
%\item relation to other things:
%\subitem Our approach shares with other theories of implicature the need to place such restrictions on the set of relevant alternatives.
%\subitem Relation to IBR and game-theoretic pragmatics. (Franke, Jaeger, Benz.)
%\end{itemize}
...
%, without specifying how \lstinline[mathescape]{$\theta$} is bound in sentences such as \emph{Bob is strong}.
%We could instead define \sv{strong} = \lstinline[mathescape]{($\lambda$ (x) (>= (strength x) $\theta$))}, allowing \lstinline[mathescape]{$\theta$} to be a free semantic index variable.\footnote{We would then have to make a corresponding change
%to the meaning of \emph{-est}: it would first bind the threshold variable locally, then proceed as above to saturate the threshold.}
...
\todo[inline]{Need an extended discussion of why and when to lift a free variable. Show the scaling properties of level-0 and level-1 variable inference...}

To understand this distribution, we can look at 
%figure \ref{strong-L0}, which plots 
...
\todo[inline]{Shalom: The paragraph immediately preceeding Section 4.1 is unclear. Please reformulate it. Shalom: FN 9 needs to be reformulated, as it is unclear.}
...
When we do so, the uniform prior distribution on interpretations of this variable changes to a decidedly non-uniform posterior distribution (figure \ref{L1-adjective-interp}, dotted line). \todo{Shalom: Are you sure that you mean figure 4 here? If so, which dotted line in that figure are referring to?}
...
\todo[inline]{Shalom: You may want to refer to Peter Sutton's work on vagueness and the Sorities paradox here. He proposes a similar approach.}
...
\todo[inline]{Noah: fill in indices related work.}

\todo{point out that index version is easily compositional. mention roses and flowers paper.}
...
%\item  Determiners? "the" vs "a": same basic meaning, but the variable from "the" is lifted to L1, hence is influenced by salience. (competition between "a" and "the"?)
...
%\todo[inline]{Shalom: it seems to me that you can dispense with the appendices A-C, and replace them with references to published work. They do not really contribute to the clarification of the points that you make in the chapter, and omitting them will leave more room for additional explanation and illustration of the technical material in the body of the text.}
...
%Gradable adjectives:
%\begin{itemize}
%\item \sv{strong} = \lstinline[mathescape]{($\lambda$ (x) ($\lambda$ ($\theta$) (>= (strength x) $\theta$)))}
%\item \lstinline{(define (weakness x) (- 0 (strength x))))}
%\item \sv{weak} = \lstinline[mathescape]{($\lambda$ (x) ($\lambda$ ($\theta$) (>= (weakness x) $\theta$)))}
%\end{itemize}
%The weakness scale is simply the strength scale with the sign reversed.
%
%Quantifiers and the superlative morpheme:
%\begin{itemize}
%\item \lstinline{(define individuals (append player team match))}
%\item \sv{every} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (all (map ($\lambda$ (x) (not (and (P x) (not Q x)))) individuals))))}
%\item \sv{some} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (some (map ($\lambda$ (x) (and (P x) (Q x))) individuals))))}
%\item \sv{most} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (Q) (> (proportion-true (map ($\lambda$ (x) (and (P x) (Q x))) individuals)) .5)))}
%\item \sv{\text{-}est} = \lstinline[mathescape]{($\lambda$ (A) ($\lambda$ (N) ($\lambda$ (x) (= (argmax A x) (argmax ($\lambda$ ($\theta$) (some (map ($\lambda$ (x) (>= (A x) $\theta$)) N))))))))}
%%\item \sv{the} = \lstinline[mathescape]{($\lambda$ (P) ($\lambda$ (x) (P x)))} 
%\end{itemize}
%We have had to extend the intuitive theory with a domain of discourse capturing all the entities constructed.
