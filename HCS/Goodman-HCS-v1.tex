...
\institute{%
   Stanford University\\
    \texttt{\{ngoodman,danlassiter\}@stanford.edu}
 }
...
\citet{goodman08} introduce Church, an enriched SLC that can be realized as a \emph{probabilistic programming language}---parallel to how the programming language LISP is an enriched lambda calculus. 
...
We next describe these key pieces of the language semantics; further details and examples of using Church for cognitive modeling can be found at \url{http://probmods.org}.
...
%(define coin )
%\end{lstlisting}
Multiple calls to the function will result in a sequence of values which are not i.i.d., because they jointly depend on the unknown \lstinline{bias} (though they are exchangeable---\citet{de1993probabilita}).
\todo{Correct citation for de finetti? Probably cite a summary, such as he one by diaconis. Original thm was 1930s, but there doesn't seem to be any cite...}
...
\begin{lstlisting}[mathescape]
(define conditional 
   ($\lambda$ () (($\lambda$ (val) (if (condition val) val (conditional))) (thunk))))
\end{lstlisting}
where we have used a stochastic recursion (conveniently specified by the named define) to build a conditional. 
Conceptually this recursion samples from \lstinline{thunk} until a value is returned that satisfies \lstinline{condition}; it is straightforward to show that the distribution over return values from this procedure is exactly the ratio used to define conditional probability in equation \ref{conditionaldef} (where both are defined).
...
(query
  ...
  condition)
\end{lstlisting}
where \lstinline{...definitions...} is a list of definitions, \lstinline{qexpr} is the expression of interest whose value we want, and \lstinline{condition} is a condition expression that must return \lstinline{true}. 
...
\lstinline{gender}, in turn, is defined %in figure \ref{theory} 
...
\todo[inline]{Noah: prob semantics related work.}
...
That is, the listener can reasonably infer from the speaker's choice not to make the stronger claim that it is probably false.
...
to the meaning of \emph{-est}: it would first bind the threshold variable locally, then proceeded as above to saturate the threshold.}
...
Instead, our literal listener maximizes the probability of the utterance's truth by setting the free variable in its meaning to a value so low that the utterance will be true for virtually any height that Bob might plausibly have. 
...
That is, an individual with strength 3 will have a roughly equal chance of counting as ``tall'' and as ``not tall''.  
...
This account of the sorites builds on previous probabilistic approaches to vagueness and the sorites \citep{borel07,black37,edgington97,lawry08,frazeebeaver10,egre11,lassiter11,egrebarberousse13}. However, it is the first to connect the probabilistic approach with a specific, cognitively and communicatively motivated account of why vague adjectives should have context-sensitive probabilistic interpretations and of how the distribution is determined in a particular context of utterance.
